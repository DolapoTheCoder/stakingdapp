...
describe('Staking', function() {
    ...
    describe('stakeEther', function() {
        it('transfer ether', async function() {
            ...
            //test that change in ithe Signer1's ether balance
            expect(
            ...
            //test the change in contracts ether balance
            expect(
        ...
        it('adds a position to posisitons', async function() {
            const provider = waffle.provider;
            let position;
            ...
            //testing position to be nothing
            ...
            expect(position.unlockedDate).to.equal(0)
        ...
        it('making a position', async function() {
            const provider = waffle.provider;
            let position;
            const transferAmount = ethers.utils.parseEther('1.0')
            ...
            expect(position.unlockedDate).to.equal(block.timestamp + (86400 * 90))
        ...
        it('add address and positionID to positionIdsByAddress', async function() {
    ...
    describe('modifyLockPerionds', function() {
        describe('owner', function() {
            it('should create a new lock period', async function() {
                await staking.connect(signer1).modifyLockPeriod(100, 999);
            ...
            it('should modify existing lock period', async function() {
                await staking.connect(signer1).modifyLockPeriod(30, 150);
        ...
        describe('non-owner', function() {
            it('reverts', async function() {
                expect(
                    staking.connect(signer2).modifyLockPeriod(100, 999)
                ).to.be.revertedWith(
                    'only owner can mofidy staking period'
                )
            })
        })
    })

    describe('get lock periods', function() {
        it('returns lock periods', async () => {
    ...
    describe('getPositionById', function() {
        it('returns data about a specific position, given a positionId', async () => {
            ...
            const position = await staking.connect(signer1.address).getPositionId(0)
    ...
    describe('getPositionIdsForAddress', function() {
        it('returns an address for ID', async () => {
            let data;
            let transaction;
            ...
            const positionsIds = await staking.getPositionIdForAddress(signer1.address)

            expect(
                positionsIds.map(p => Number(p._hex))
            ).to.eql(
                [0,1]
            )
        })
    ...
    describe('changeUnlockDate', function() {
        describe('owner', function() {
            it('changes the unlockDate', async () => {
                ...
                const positionOld = await staking.getPositionId(0)
                
                const newUnlockDate = positionOld.unlockedDate - (86400*500)
                await staking.connect(signer1).changeUnlockDate(0, newUnlockDate)
                const positionNew = await staking.getPositionId(0)

                expect(
                    positionNew.unlockedDate
                ).to.be.equal(
                    positionOld.unlockedDate - (86400*500)
                )
            })
        })
        describe('non-owner', function() {
        ...
        describe('closePosition', function() {
            describe(' after unlock date', function() {
                it('transfers initial and interest', async () => {
                    let transaction;
                    let receipt;
                    let block;
                    const provider = waffle.provider;
                    ...
                    //change unlock time to past
                    const newUnlockDate = block.timestamp - (86400*100)
                    await staking.connect(signer1).changeUnlockDate(0, newUnlockDate);
                    
                    const position = await staking.getPositionId(0)
